  ...
  return (
    <div className="border rounded-2xl p-3 space-y-2">
      ...
      <input
        ...
        value={title}
        onChange={(e)=>{ setTitle(e.target.value); onChange && onChange({ title: e.target.value, file }); }}
      />
      <div className="flex items-center gap-3">
        <input ref={inputRef} type="file" accept="image/*" onChange={(e)=>handleFile(e.target.files?.[0])} className="hidden" />
        <button type="button" onClick={()=>inputRef.current?.click()} className="px-3 py-2 rounded-xl border bg-white hover:bg-gray-50">
          Cargar foto
        </button>
